...
- **🔐 Disk Encryption (experimental)**: LUKS on automatic layouts with non-interactive `cryptsetup` (passphrase via stdin so logs stay clean), early `dm-crypt` setup, mkinitcpio hooks (`sd-encrypt` or classic `encrypt` as needed), and bootloader kernel options that match the hook style (`rd.luks.name=` vs `cryptdevice=` / `root=`) for systemd-boot / GRUB when encryption is on
...
#### 6. **Bootloader**
- systemd-boot (UEFI only; the installer blocks this choice on BIOS/legacy firmware—use GRUB there)
