...
pub fn draw_configuration(frame: &mut ratatui::Frame, app: &mut AppState, area: Rect) {
    ...
    for (label, idx) in options {
        ...
        let is_active_line = is_focused_line && matches!(app.focus, crate::app::Focus::Content);
    ...
    let continue_style =
        if app.config_focus_index == 2 && matches!(app.focus, crate::app::Focus::Content) {
            Style::default()
                .fg(Color::Yellow)
                .add_modifier(Modifier::BOLD)
        } else {
        ...
        .block(
            Block::default()
                ...
                .title(match app.focus {
                    crate::app::Focus::Content => " Desicion Menu (focused) ",
                    _ => " Desicion Menu ",
                }),
        )
