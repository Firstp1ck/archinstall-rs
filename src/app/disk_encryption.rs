...
impl AppState {
    ...
    fn collect_partitions(device_filter: Option<&str>) -> Vec<String> {
        ...
        if device_filter.is_none() {
            return lines;
        }
        if let Ok(output) = Command::new("lsblk")
            .args(["-J", "-b", "-o", "NAME,PATH,TYPE,SIZE"])
            .output()
...
pub fn draw_disk_encryption(frame: &mut ratatui::Frame, app: &mut AppState, area: Rect) {
    ...
    let is_focused_line =
        app.diskenc_focus_index == 0 && matches!(app.focus, super::Focus::Content);
    let bullet = if app.diskenc_focus_index == 0 {
        "▶"
    } else {
        " "
    };
    let bullet_style = if is_focused_line {
        Style::default()
            .fg(Color::Yellow)
            .add_modifier(Modifier::BOLD)
    } else {
    ...
    let label_style = if is_focused_line {
        Style::default()
            .fg(Color::Yellow)
            .add_modifier(Modifier::BOLD)
    } else {
    ...
    let value_style = if is_focused_line {
        Style::default()
            .fg(Color::Yellow)
            .add_modifier(Modifier::BOLD)
    } else {
    ...
    if app.disk_encryption_type_index == 1 {
        ...
        lines.push(Line::from(vec![
            Span::styled(
                format!(
                    "{} ",
                    if app.diskenc_focus_index == idx {
                        "▶"
                    } else {
                        " "
                    }
    ...
    let continue_style = if app.diskenc_focus_index == continue_index
        && matches!(app.focus, super::Focus::Content)
    {
        Style::default()
            .fg(Color::Yellow)
            .add_modifier(Modifier::BOLD)
