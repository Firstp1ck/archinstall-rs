    ...
    match ev {
        Event::Key(key) if key.kind == KeyEventKind::Press => match key.code {
            ...
            KeyCode::Enter => {
                if app.focus == Focus::Menu {
                    app.info_message =
                        format!("Opened '{}'", app.menu_entries[app.selected_index].label);
                    ...
                    } else if app.selected_index + 1 < app.menu_entries.len() {
                        app.selected_index += 1;
                        app.list_state.select(Some(app.selected_index));
                        app.focus = Focus::Content;
                        if app.current_screen() == Screen::Locales {
...
fn change_bootloader_value(app: &mut AppState, _next: bool) {
    if app.current_screen() != Screen::Bootloader || app.focus != Focus::Content {
        return;
    }
    if app.bootloader_focus_index < 4 {
