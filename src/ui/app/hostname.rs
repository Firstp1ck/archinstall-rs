...
pub fn draw_hostname(frame: &mut ratatui::Frame, app: &mut AppState, area: Rect) {
        ...
        let value_style = if is_active_line { Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD) } else { Style::default().fg(Color::White) };
        let current = if app.hostname_value.is_empty() { "<empty>".into() } else { app.hostname_value.clone() };
