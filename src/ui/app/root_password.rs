...
pub fn draw_root_password(frame: &mut ratatui::Frame, app: &mut AppState, area: Rect) {
    ...
    // Continue
    let continue_style = if app.rootpass_focus_index == 2 && matches!(app.focus, Focus::Content) { Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD) } else { Style::default().fg(Color::White) };
