                ...
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                    ...
                    if let Some(blockdevices) = json.get("blockdevices").and_then(|v| v.as_array()) {
                        for dev in blockdevices {
                            let devtype = dev.get("type").and_then(|v| v.as_str()).unwrap_or("").to_string();
                            if devtype != "disk" { continue; }
                            let mut model = dev.get("model").and_then(|v| v.as_str()).unwrap_or("").trim().to_string();
                            let name = dev.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string();
                            if model.is_empty() { model = name.clone(); }
                            let path = dev.get("path").and_then(|v| v.as_str()).map(|s| s.to_string()).unwrap_or_else(|| format!("/dev/{}", name));
                            ...
                            if let Some(children) = dev.get("children").and_then(|v| v.as_array()) {
                                for ch in children {
                                    if let Some(sz) = ch.get("size").and_then(|v| v.as_u64()) { used_b = used_b.saturating_add(sz); }
                                }
                            }
                            let free_b = size_b.saturating_sub(used_b);
                            let size = Self::human_bytes(size_b);
                            let freespace = if size_b == 0 { "-".into() } else { Self::human_bytes(free_b) };
                            ...
                            devices.push(DiskDevice { model, path, devtype, size, freespace, sector_size, read_only: ro });
                        }
                    }
