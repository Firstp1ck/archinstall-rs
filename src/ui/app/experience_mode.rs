...
impl AppState {
    #[allow(dead_code)]
    pub fn init_experience_mode(&mut self) {
        // nothing to init currently
    }

    #[allow(dead_code)]
    pub fn current_experience_mode_label(&self) -> &'static str {
        match self.experience_mode_index {
            0 => "Desktop",
            1 => "Minimal",
            2 => "Server",
            _ => "Xorg",
...
pub fn draw_experience_mode(frame: &mut ratatui::Frame, app: &mut AppState, area: Rect) {
    ...
    let choices = ["Desktop", "Minimal", "Server", "Xorg"];

    for (idx, label) in choices.iter().enumerate() {
        let is_focused_line = app.experience_focus_index == idx;
        let is_active_line = is_focused_line && matches!(app.focus, Focus::Content);
        let is_selected = app.experience_mode_index == idx;
    ...
    let continue_style = if app.experience_focus_index == 4 && matches!(app.focus, Focus::Content) {
        Style::default()
            .fg(Color::Yellow)
            .add_modifier(Modifier::BOLD)
    } else {
